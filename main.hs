...
main = do
    ...
    flip runStateT state (do 
        let start = distortion 0.8 $ mkSinusoide (\x -> x) (\t -> (note C 3) - ((note C 3) -(note C 1))*t*t) 0.2
        ...
        noise <- liftIO $ whiteNoise 1.0
        liftIO $ putStrLn $ show $ length noise
        play noise
        --liftIO $ plot $ whiteNoise 1
        )
    simpleDrain simple
    simpleFree simple